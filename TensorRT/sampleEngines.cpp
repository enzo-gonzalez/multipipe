...
public:
    RndInt8Calibrator(
            int batches, std::vector<int64_t>& elemCount, const std::string& cacheFile, const nvinfer1::INetworkDefinition& network, std::ostream& err);
...
RndInt8Calibrator::RndInt8Calibrator(
        int batches, std::vector<int64_t>& elemCount, const std::string& cacheFile, const INetworkDefinition& network, std::ostream& err)
    : mBatches(batches)
    , mCurrentBatch(0)
                           ...
                           std::vector<std::vector<char>>& sparseWeights)
{
    ...
    else if (build.int8)
    {
        if (!hasQDQLayers(network) && int8IO)
        {
            ...
            {
                sample::gLogError << "Int8IO was specified but impossible to read tensor scales from provided calibration cache file"
                                  << std::endl;
                return false;
            }
        }
...
//        return nullptr;
//    }
    QFile  file(QString::fromStdString(engine));
    if(!file.open(QFile::ReadOnly))
    {
        err << "Error opening engine file: " << engine << std::endl;

    }
     err << "opening ok"  << std::endl;
...
//    }
    ...
    runtime->setErrorRecorder(&gRecorder);
   return runtime->deserializeCudaEngine(data.data(), data.length(), nullptr);
...
{
public:
    ScopeMuter(Logger& logger)
        : mLogger(logger)
        , mOrigSeverity{logger.getReportableSeverity()}
    {
        mLogger.setReportableSeverity(static_cast<ILogger::Severity>(-1));
    }

    ~ScopeMuter()
    {
        mLogger.setReportableSeverity(mOrigSeverity);
    }
private:
    Logger& mLogger;
    const ILogger::Severity mOrigSeverity;
};
...
bool timeRefit(const INetworkDefinition& network, nvinfer1::ICudaEngine& engine)
{
    ...
    time_point const refitStartTime{std::chrono::steady_clock::now()};
    {
        for (int32_t l = 0; l < loop; l++)
        {
            ...
            // mute scope
            {
                ScopeMuter muteLogger(sample::gLogger);
                for (int32_t i = 0; i < nbLayers; i++)
                {
                    const auto layer = network.getLayer(i);
                    const auto roleWeightsVec = getAllRefitWeightsForLayer(*layer);
                    for (const auto& roleWeights : roleWeightsVec)
                    {
                        const bool success
                                = refitter->setWeights(layer->getName(), roleWeights.first, roleWeights.second);
            ...
            const auto refitSuccess = refitter->refitCudaEngine();
            if (!refitSuccess)
            {
                return false;
            }
    ...
    const time_point refitEndTime{std::chrono::steady_clock::now()};
