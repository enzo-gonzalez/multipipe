...
struct Binding
{
    ...
    void preProcess(cv::Mat img, float *mdata) {

//        QVector<float> result;
        int imgSize=224;
        std::vector<float> v1;
        cv::Mat Bands[3];
        cv::Size scale(imgSize,imgSize);
        int cnt_i=0;

        //        float *mData;
        //        mData=new float[imgSize*imgSize*3];
        ...
        for (int c = 0; c < 3; ++c) {
            v1.assign((float*)Bands[c].datastart, (float*)Bands[c].dataend);
...
//        cv::Mat img2 = cv::imread("/home/amin/Desktop/testtrt/lib/TensorRT-8.0.0.3/data/mnist/perfect.png");


//        QVector<float> imgData = preProcess(img);
//        QVector<float> imgData2 = preProcess(img2);



//        float*ptr=(float*)buffer.getHostBuffer();

//        for (int i = 0; i < imgSize * imgSize * 3; i++)
//        {
//            ptr[i] =imgData.at(i);
...
//        for (int i = 0; i < imgSize * imgSize * 3; i++)
//        {
//            ptr[i+(imgSize * imgSize * 3)] = imgData2.at(i);
...
//        //            file.close();
//        //        }
//        //        uint8_t* buff=new uint8_t[28*28];
...
//        //        infile.read(reinterpret_cast<char*>(buff), 28 * 28);

//        //        float*ptr=(float*)buffer.getHostBuffer();
//        //        float test[28 * 28];
//        //        for (int i = 0; i < 28 * 28; i++)
//        //        {
//        //            ptr[i] =1.0 - float(buff[i] / 255.0);
//        //            test[i]=((float*)buffer.getHostBuffer())[i];
//        //        }

//        //        std::cout<<"index[20]=";

    }
    void fillMat( QVector<VisionObject>  objects)
    {
        float*ptr=(float*)buffer.getHostBuffer();
        int32_t *intptr=(int*)buffer.getHostBuffer();

        float mData[224*224*3];
        ...
        {
//            QVector<float> imgData = preProcess(objects[i].Image);
//            preProcess(objects[i].Image, mData);
            preProcess(objects[i].Image, (float*)intptr);

//            std::memcpy(intptr, mData, ( 224*224*3 * sizeof(float)));
            intptr+=(224*224*3);
...
{
public:
    void Allocation(int b, const std::string& name, bool isInput, int64_t volume,
                    ...
                    const std::string& fileName = "")
    {
       ...
       // qDebug()<<" ------ garbage time: "<<std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count()<<" ms";
        ...
        if (isInput)
        {
            ...
            else
            {
               // auto startClock = std::chrono::high_resolution_clock::now();
                ...
                //qDebug()<<" Fill time: "<<std::chrono::duration_cast<std::chrono::milliseconds>(endClock - startClock).count()<<" ms";
            }
        }
    }
    ...
    void fillImage(int binding,  QVector<VisionObject>  objects)
    {
        mBindings[binding].fillMat(objects);
